...
- Companion Roci work: a later, separate Roci branch for framework changes
...
Streamlit app, keeps Portico's TOML calculation settings, and moves page layout,
filters, and chart choices into TOML as well.
...
- The current Portico app has Home plus nine page modules, and its
  ...
  budgets, subscriptions, safety, and financial-independence inputs.
...
| Preserve Portico's calculation TOML and add typed dashboard TOML | Accepted | Existing financial settings remain familiar; dashboard configuration removes hard-coded page presentation without making TOML executable. | [SADD: Configuration](SADD.md#configuration-and-cli) |
| Use a left slide-out page drawer | Accepted | It is the requested navigation behaviour and exposes a missing reusable Roci component. | [SADD: Desktop shell](SADD.md#desktop-shell-and-navigation) |
...
- The companion Roci branch may add public `Roci.Ui` components for a navigation
  drawer, temporal/category chart data, range bars, and heatmaps. Each addition
  follows Roci's fluent `UiBuilder` language and includes its own sample and
  tests.
