...
class TestTransactionsScrub:
    ...
    def test_scrub_drops_unnamed_column(
        self, scrub_input_transactions_df: pd.DataFrame, categories_for_scrub: CategoriesSpreadsheet
    ) -> None:
...
class TestBalanceScrub:
    ...
    def test_scrub_missing_unnamed_column(
        self, scrub_input_balance_df: pd.DataFrame, accounts_for_scrub: AccountsSpreadsheet
    ) -> None:
        """scrub() should not crash when 'Unnamed: 0' column is absent."""
    ...
    def test_scrub_joins_group_from_accounts(
        self, scrub_input_balance_df: pd.DataFrame, accounts_for_scrub: AccountsSpreadsheet
    ) -> None:
    ...
    def test_scrub_filters_hidden(
        self, scrub_input_balance_df: pd.DataFrame, accounts_for_scrub: AccountsSpreadsheet
    ) -> None:
        bs = self._make(scrub_input_balance_df, accounts_for_scrub)
        assert "Hidden" not in bs.scrubbed_df["Account"].values

    def test_scrub_balance_float(
        self, scrub_input_balance_df: pd.DataFrame, accounts_for_scrub: AccountsSpreadsheet
    ) -> None:
        bs = self._make(scrub_input_balance_df, accounts_for_scrub)
...
class TestLoadErrorHandling:
    def test_load_calls_st_stop_on_failure(self) -> None:
        ...
        with (
            ...
            assert mock_error.call_args[0][0] == "Failed to load remote spreadsheet data (transactions)."
