...
public sealed class TomlConfigurationReaderTests
{
    ...
    public async Task LocalFile_MapsAllPolicyGroups()
    {
        ...
        ReportChoiceSettings choices = result.ReportChoices;
        Assert.Equal([3, 6, 12, 24], choices.Lookback.Months);
        Assert.Equal(12, choices.Lookback.DefaultMonths);
        ...
        Assert.Equal("All spending", choices.TransactionSetLabels["all"]);
        Assert.Equal(["all"], choices.FilterSet("spending").Options);
        Assert.Equal("all", choices.FilterSet("year_over_year").Default);
        Assert.True(choices.DefaultIncomeIsRegular);
        ...
        Assert.Equal(["One-off"], choices.DefaultSubscriptionDiscoveryExclusions);
