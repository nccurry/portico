...
def find_duplicates_efficient(
    ...
    candidates = df[(df["Amount"].abs() >= min_amount) & df["Date"].notna()].copy()
    if candidates.empty:
        return pd.DataFrame(columns=_DUPLICATE_COLUMNS)

    candidates = candidates.sort_values(["Amount", "Date"]).reset_index(drop=True)
    candidates["_norm_desc"] = candidates["Full Description"].apply(normalize_description)
        ...
        for left_position, left in enumerate(records):
            for right in records[left_position + 1 :]:
                ...
                if check_same_category and left["Category"] != right["Category"]:
                    continue
                if require_same_description and left["_norm_desc"] != right["_norm_desc"]:
                    continue
                pairs.append(
                    {
                        "Date1": left["Date"],
...
def summarize_duplicates(duplicates: pd.DataFrame) -> DuplicateSummary:
    ...
    return DuplicateSummary(
        ...
        total_amount=float(duplicates["Amount"].abs().sum()) if not duplicates.empty else 0.0,
        affected_months=int(duplicates["Month"].nunique()) if not duplicates.empty else 0,
    )
...
def summarize_duplicates_by_month(duplicates: pd.DataFrame) -> pd.DataFrame:
    ...
    monthly = (
        duplicates.groupby("Month")["Amount"]
        .agg(
            Count="count",
