...
public sealed record CsvTable(IReadOnlyList<string> Headers, IReadOnlyList<IReadOnlyDictionary<string, string>> Rows)
{
    ...
    public static CsvTable Parse(string content, string sourceName)
    {
        ...
        for (int rowIndex = 1; rowIndex < records.Count; rowIndex++)
        {
            ...
            if (values.Count != headers.Length)
                throw new DataLoadException($"{sourceName} row {rowIndex + 1} has {values.Count} columns; expected {headers.Length}.");
    ...
    public void RequireHeaders(string sourceName, params string[] headers)
    {
        foreach (string header in headers)
        {
            if (!Headers.Contains(header, StringComparer.OrdinalIgnoreCase))
                throw new DataLoadException($"{sourceName} is missing required column '{header}'.");
        }
    }
}
...
{
    /// <summary>Creates a safe data-loading exception.</summary>
...
internal static class CsvRecords
{
    public static IReadOnlyList<IReadOnlyList<string>> Parse(string content, string sourceName)
    {
        ...
        if (quoted)
            throw new DataLoadException($"{sourceName} has an unterminated quoted field.");
