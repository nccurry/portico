...
internal sealed class PorticoDataHealthPageRenderer
{
        ...
        Func<string?, string> display)
    {
        ...
        DataHealthCheckView selected = view.SelectedCheck;
    ...
    private void BuildSelectedCheckControl(UiBuilder ui, DashboardPageDefinition page, DataHealthPageView view)
    {
        ...
        {
            DataHealthCheckView check = view.Analysis.Checks.Single(check => string.Equals(check.Id, option, StringComparison.Ordinal));
    ...
    private static string StatusLabel(DataHealthCheckView check)
        => check.Status switch
        {
            "Passed" => "Passed",
